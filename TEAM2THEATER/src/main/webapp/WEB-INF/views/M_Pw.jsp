                            ...
                            <div class="col-lg-6">
                                <div class="p-5">
                                    ...
                                    <form class="user" action="mPwChange" method="post" name="mPwForm">
										...
										<div class="form-group" id="mPwChange">
...
<script>
		...
		$.ajax({
			...
			success : function(uuid) {
				...
				$("#confEmail").html("<br/><input type='text' id='uuidCheck'/>"
						...
						+" <input type='button' value='인증' onclick=\"checkEmail2()\" style='height: 30px; width: 100px; background-color: red; color:white; border-radius: 150px;'/>");
			},
			error : function() {
				alert('이메일 인증 통신 실패!');
			}
  ...
  function checkEmail2(){
		...
		if(uuidCheck==null || uuidCheck==""){
			...
			alert('인증성공!');boolUUID = true;
			$("#mPwChange").html("<br/><input type='password' id='memPw' name='memPw' onkeyup='pwRegexp()' placeholder='변경할 비밀번호(영문, 숫자, 특수문자 혼합!)'/><br/> <span id='pwResult1'></span>"
					+"<div><input id='loginButton' type='submit' value='비밀번호 변경' style='width:90; height:40px; line-height:40px; text-center:center; border-radius:150px; background-color:red; color:white;'></div>");
		} else {
			alert('인증실패!');
		}
	...
	//이거 안 되면 span문제일 수 있으니 div는 다시 form으로 옮기기
