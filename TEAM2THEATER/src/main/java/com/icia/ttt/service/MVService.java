...
public class MVService {
	...
	public ModelAndView mvRes(MOVIE movie) {

		int mvCount = mvdao.mvCount(movie);

		System.out.println("mvCount : " + mvCount);

		String mvRes = mvdao.mvRes(mvCount);
		
		System.out.println("mvRes : " + mvRes);
	...
	public ModelAndView mvView(int mpseq) {
		MOVIEPOST movie = mvdao.mvView(mpseq);
		System.out.println(movie);

		mav.setViewName("MV_View");
		mav.addObject("moviepost", movie);
...
public MOVIELIKE mlCheck(MOVIELIKE movieLike) {
	...
	public ModelAndView mvModify(MOVIEPOST movie) throws IllegalStateException, IOException {
		...
		if (result > 0) {
			...
			mav.setViewName("redirect:/MV_List");
		}
